...
class LlamaForSequenceClassificationPerLayer(LlamaPreTrainedModel):
    """Copy of LlamaForSequenceClassification but uses the output of the k layer for the classification.
    ...
    def forward(
        ...
        # Original code: hidden_states = transformer_outputs[0]
...
class T5HiddenStatesClassifier(nn.Module):
    def __init__(self, model, hidden_size, num_labels, hidden_states_from_layer=-1):
        super().__init__()
        self.num_labels = num_labels
        self.model = model
        self.model.eval()
        self.linear = nn.Linear(hidden_size, self.num_labels, bias=False)
        self.hidden_states_from_layer = hidden_states_from_layer
    ...
    def forward(self, input_ids, labels):
        ...
        logits = self.linear(
            outputs.decoder_hidden_states[self.hidden_states_from_layer][:, 0, :]
        )
        loss = None
        if labels is not None:
            labels = labels.to(logits.device)
